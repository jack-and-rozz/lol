...
import sys, os, time, re, math, collections, json, yaml
...
def multi_process(func, *args, kwargs=None):
    ...
    if kwargs is None:
        kwargs = [{} for _ in range(len(args[0]))]

    # kwargs are not supported... (todo)
    for i, a in enumerate(zip(*args)):
        worker = mp.Process(target=wrapper(func, i, q), args=a, kwargs=kwargs[i])
...
def modelname_converter(model_name):
    model_name = '.'.join(model_name.split('.')[1:])
    modelnames_dict = yaml.load(open('configs/modelname_converter.yaml'), 
                                Loader=yaml.FullLoader)
    model_name = model_name if model_name not in modelnames_dict else modelnames_dict[model_name]
    return model_name
